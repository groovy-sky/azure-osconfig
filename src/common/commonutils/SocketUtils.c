...
int ReadHttpContentLengthFromSocket(int socketHandle, void* log)
{
    ...
    const char* contentLengthLabelLower = "content-length: ";
    ...
    if (NULL != buffer)
    {
        if (NULL != (contentLength = strstr(buffer, contentLengthLabel)) ||
            NULL != (contentLength = strstr(buffer, contentLengthLabelLower)))
        {
